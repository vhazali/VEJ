...
for k, v in location_list.items():
	...
	if os.path.isfile(sdk_file): # Continue if sdk exists
		proc = subprocess.Popen(["java", "-Xmx4g", "-cp", "flowdroid/soot-trunk.jar;flowdroid/soot-infoflow.jar;flowdroid/soot-infoflow-android.jar;flowdroid/slf4j-api-1.7.5.jar;flowdroid/slf4j-simple-1.7.5.jar;axml-2.0.jar", "soot.jimple.infoflow.android.TestApps.Test", apk_file, sdk_file, "--saveresults", filename], stderr=subprocess.STDOUT, stdout=subprocess.PIPE).communicate()[0]
